...
- [ ] Add an email address to the `ADMINS` settings variable in `EmplayWeb/backend/EmplayWeb/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `EmplayWeb/backend/EmplayWeb/settings/production.py`
...
- Inside the `backend` folder, do the following:
  - Create a copy of `EmplayWeb/settings/local.py.example`:  
  `cp EmplayWeb/settings/local.py.example EmplayWeb/settings/local.py`
...
- Open a new command line window and go to the project's directory.
- Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv EmplayWeb` or `python -m venv EmplayWeb-venv`
...
- Make sure the virtualenv is activated `workon EmplayWeb` or `source EmplayWeb-venv/bin/activate`
...
- `workon EmplayWeb` or `source EmplayWeb-venv/bin/activate` depending on if you are using virtualenvwrapper or just virtualenv.
